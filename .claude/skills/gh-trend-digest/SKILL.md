...
**같은 레포 리뷰가 이미 있으면 작업하지 않는다.** 사용자가 모르고 같은 URL을 다시 던졌을 수 있고, 의도된 재작성이면 명시적 키워드를 줄 것이다.

판정 절차:

1. **사용자 메시지에 후속 키워드가 있는가?**
   - 있음 (예: "다시", "재작성", "재실행", "톤", "보완", "최신", "수정", "추가", "갱신") → 0-3 컨텍스트 확인으로 진행
   - 없음 → 다음 단계

2. **결과 파일 존재 여부 확인:**
   ```bash
   # categories/ 하위 어느 언어 디렉토리든 같은 repo 이름의 글 검색
   find categories -type f -name '{repo}.md' -o -name '{repo}-*.md' 2>/dev/null
   ```
   - 매치된 파일이 있으면 **frontmatter의 `repo:` 필드를 확인**해 `{owner}/{repo}`가 정확히 일치하는지 검증
   - 정확히 일치 → **중단**. 사용자에게 보고:
     ```
     ℹ️ 이미 리뷰가 있어요: categories/{language}/{repo}.md
     - 작성일: {frontmatter.date}
     - 분량: {N}자
     다시 쓰려면 "톤 가볍게 다시", "최신 반응 추가", "재작성" 등 후속 키워드와 함께 요청해주세요.
     ```
   - 동명이인(같은 이름, 다른 owner) → 정보 표시 후 사용자 확인: "{기존 owner}/{repo}는 이미 있어요. 새 owner의 같은 이름 레포로 진행할까요?" → 대기
   - 없음 → 0-3 컨텍스트 확인으로 진행

### 0-3. 컨텍스트 확인 (재실행 모드)

후속 키워드가 있거나, 중복 체크를 통과한 경우 작업 모드를 결정:
